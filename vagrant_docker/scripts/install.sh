...
function isinstalled {
  if dpkg --list-installed "$@" >/dev/null 2>&1; then
    true
  else
    false
  fi
...
function install_configure_packages {
    ...
    #sudo apt-get install -y --no-install-recommends haproxy keepalived pgbouncer git openssl curl wget net-tools psmisc tcpdump sipcalc
